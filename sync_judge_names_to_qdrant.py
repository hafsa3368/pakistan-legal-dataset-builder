"""
sync_judge_names_to_qdrant.py
===============================
Same pattern as sync_case_numbers_to_qdrant.py, for the judge field
instead of case_number -- with one deliberate difference, learned from a
near-miss while building the Neo4j counterpart (sync_judge_names_to_neo4j.py):

case_number is a flat property with one source of truth (the JSON), so
"sync whatever's in the JSON" is safe for it. judge is NOT -- JSON files
only got rewritten by fix_judge_names_json.py where their OWN value was
invalid; a JSON value that was "valid-looking but messy" (a typo, odd
casing) was left as-is even where Qdrant/Neo4j might already hold a
cleaner version from other cleanup passes. A dry run of a naive
"sync whatever differs" version of the Neo4j script would have overwritten
clean canonical names with typos, and reintroduced garbage
(fix_judge_names.py had already deleted) that fix_judge_names_json.py
never actually managed to fix for that file.

So this script, unlike its case_number sibling:
  - Re-cleans and re-validates the JSON's judge value with
    clean_judge_candidate() / is_valid_judge_name() (imported directly
    from new_extractor.py) before ever considering it.
  - Only fills genuine gaps: a point's existing "judge" payload value is
    NEVER overwritten if it's already non-empty. This mirrors the
    Neo4j script's "gap-fill only" policy.
"""
...
CHECKPOINT_FILE = "sync_judge_names_qdrant_checkpoint.json"
...
# =========================================================
# BUILD filename -> judge MAP FROM JSON (source of truth)
# =========================================================

print("Building filename -> judge map from in-scope JSON files "
...
for path in json_files:
    ...
    if not isinstance(judge_raw, str):
        continue
    judge = clean_judge_candidate(judge_raw.strip())
    if not is_valid_judge_name(judge):
        continue
    ...
    if generated_name:
        by_generated_name[generated_name] = judge
    if actual_filename:
        by_actual_filename.setdefault(actual_filename, judge)
...
print(f"Loaded {len(by_generated_name)} generated_name entries and "
      ...
      f"from {len(json_files)} in-scope JSON files.", flush=True)
...
# =========================================================
# SYNC PAYLOAD -- scoped ONLY to the in-scope filenames, using the
# indexed generated_name / actual_filename fields to filter Qdrant
# server-side instead of scrolling the whole collection.
# =========================================================
...
for chunk_idx, (field, values) in enumerate(work_items):
    ...
    while True:
        ...
        points, next_offset = client.scroll(
            ...
            limit=BATCH_SIZE,
            offset=chunk_offset,
            with_payload=["generated_name", "actual_filename", "judge"],
            with_vectors=False,
        )
        ...
        for point in points:
            ...
            new_judge = lookup.get(key)
            if not new_judge:
                skipped += 1
                continue

            # Gap-fill only -- never overwrite an existing value (see
            # module docstring for why this differs from case_number's
            # sync policy).
            old_judge = str(payload.get("judge") or "").strip()
            if old_judge:
                skipped += 1
                continue

            chunk_groups[new_judge].append(point.id)
    ...
    for group_i, (judge, point_ids) in enumerate(chunk_groups.items(), 1):
        client.set_payload(
            collection_name=COLLECTION_NAME,
            payload={"judge": judge},
            points=point_ids,
...
print("\n===================================", flush=True)
print("Qdrant judge sync completed", flush=True)
print("===================================", flush=True)
