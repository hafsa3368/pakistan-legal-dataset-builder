...
# PDF page render resolution for OCR. PyMuPDF's default get_pixmap() renders
# at ~72 DPI, which is too low for EasyOCR to read reliably on scanned
# court documents (small/dense fonts) -- this was a major source of garbled
# OCR output. 300 DPI is the standard baseline for OCR accuracy.
OCR_DPI              = 300
OCR_ZOOM             = OCR_DPI / 72
...
# ==========================
# CASE NUMBER EXTRACTION (v10)
# ==========================
# Built from 15 rounds of auditing the already-extracted corpus with a
# separate patch script (fix_case_numbers.py). Each prefix below was added
...
# lookahead guard is needed (an earlier version used one and it incorrectly
# blocked "C.O.No." because "No" itself starts with a letter).
...
def _case_number_confidence(matched_text: str) -> str:
...
def _is_well_formed_case_number(candidate: str) -> bool:
    ...
    number. Used to reject truncated fragments an LLM fallback might return
    (e.g. 'S-372 of 2019' with the prefix and 'No.' token stripped off)."""
    if not candidate:
        return False
...
def _normalize_for_compare(s: str) -> str:
    ...
    ALL whitespace. Used ONLY for the "is this value present in the
    header" comparison -- never used to store a value."""
    s = str(s or "")
    s = s.lower()
    s = s.replace("\u2013", "-").replace("\u2014", "-")
    s = re.sub(r"\s+", "", s)
...
def extract_case_number_from_header(header_text: str) -> str:
    """Tries the standard prefix list first, then the short-abbreviation
    list. Returns '' if neither matches, or if the match looks low-
    confidence (prose-like rather than a genuine caption)."""
    m = _CASE_NO_STANDARD_RE.search(header_text)
    if m:
        candidate = m.group(1).strip()
        if _case_number_confidence(candidate) == "high":
            return candidate
        return ""
...
def extract_document_metadata(full_text: str, row: dict) -> dict:
    ...
    # ── Case number ────────────────────────────────────────────────────
    # IMPORTANT: only search the HEADER of the document (first ~3000 chars).
    ...
    #
    # v10 -- prefix list expanded from 8 to 30+ formats and the header
    # window widened from 1200 to 3000 chars, based on 15 rounds of
    # auditing the already-extracted corpus (fix_case_numbers.py). See
    # _CASE_NO_STANDARD_RE / _CASE_NO_SHORT_RE near the top of this file
    # for the full prefix list and why each one was added.
    header_text = full_text[:HEADER_WINDOW]
    meta["case_number"] = extract_case_number_from_header(header_text)
    ...
    if missing:
        ...
        if "case_number" in missing:
            ...
            # v10 -- TWO safety checks now (were one before):
            # (1) candidate must appear in header_text, using the SAME
            #     normalized (whitespace-stripped, dash-normalized) check
            #     used by the cleanup pipeline -- catches cases where
            #     Ollama reproduces the number with slightly different
            #     spacing than the source and a raw substring check would
            #     have wrongly rejected it.
            # (2) candidate must be a COMPLETE, well-formed case number on
            #     its own (matches one of our prefix patterns end-to-end),
            #     not a truncated fragment. A real regression found during
            #     cleanup: Ollama once returned "S-372 of 2019" -- missing
            #     the "Cr.B.A." prefix and "No." token -- which WAS a
            #     substring of the header (so check #1 alone passed it)
            #     but was not a usable case number.
            # Match na ho to case_number khali hi rehta hai — khali behtar
            # hai ghalat/incomplete value se.
            if (candidate
                    and is_verbatim_in_header(candidate, header_text)
                    and _is_well_formed_case_number(candidate)):
